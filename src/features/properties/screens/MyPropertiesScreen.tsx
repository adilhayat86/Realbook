...
export function MyPropertiesScreen({ navigation }: Props) {
  ...
  const renderManageActions = () => {
      ...
      return (
        <>
          {renderAction({
            ...
            tone: 'neutral',
            onPress: () => setFeedback('Edit Listing is next in the build plan.'),
          })}
          {renderAction({
            icon: 'refresh-outline',
            label: 'Refresh Listing',
            helper: 'Bump this inventory as recently checked.',
            onPress: () => handleStatusChange(selectedListing, 'active', 'Listing refreshed.'),
          })}
          {renderAction({
            icon: 'checkmark-done-outline',
            label: 'Mark as Sold',
            helper: 'Move to sold records and remove from active search.',
            onPress: () => handleStatusChange(selectedListing, 'sold', 'Listing moved to Sold.'),
          })}
          {renderAction({
            ...
            tone: 'danger',
            onPress: () => handleStatusChange(selectedListing, 'archive', 'Listing archived.'),
          })}
        </>
      );
    }
      ...
      return (
        <>
          {renderAction({
            ...
            onPress: () => handleStatusChange(selectedListing, 'active', 'Listing reactivated.'),
          })}
          {renderAction({
            icon: 'archive-outline',
            label: 'Archive Listing',
    ...
    return renderAction({
      ...
      onPress: () => handleStatusChange(selectedListing, 'active', 'Listing restored to Active.'),
    });
  };
            ...
            <View style={styles.sheetHeader}>
              <View style={styles.sheetTitleWrap}>
                ...
                <Text style={styles.sheetTitle} numberOfLines={1}>
                  {selectedListing?.propertyType || 'Listing'}
                </Text>
